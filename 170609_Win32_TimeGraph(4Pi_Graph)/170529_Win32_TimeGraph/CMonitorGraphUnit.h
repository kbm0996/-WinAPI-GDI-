...
protected:
	...
	void	Paint_BarSingleVert(void);		// CPU %, 패킹처리 K TPS, 업데이트 K FPS
	void	Paint_BarSingleHorz(void);
	void	Paint_BarColumnVert(void);		// DB버퍼
	void	Paint_BarColumnHorz(void);
	void	Paint_LineSingle(void);			// 메모리MB, 네트워크 Mbyte/s
...
private:
	...
	//------------------------------------------------------
	WCHAR			_szTitle[32];
	WCHAR			_szUnitName[16]; // 자료 단위 SetInformation (szTitle szUnitName iDataMax Alert) paint_title에서 하면 됨.
